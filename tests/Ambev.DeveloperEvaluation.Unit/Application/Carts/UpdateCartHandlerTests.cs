...
public class UpdateCartHandlerTests
{
    ...
    public UpdateCartHandlerTests()
    {
        ...
        _handler = new UpdateCartHandler(_cartRepository, _productRepository, _userRepository, _mapper);
    }
    ...
    public async Task Handle_InvalidProductIdInCommand_ThrowsResourceNotFoundException()
    {
        ...
        // Given
        var command = ApplicationCartsTestData.GetValidUpdateCartCommand();
    ...
    public async Task Handle_ValidRequest_CreatesACart()
    {
        ...
        await _cartRepository.Received(1).UpdateAsync(
            Arg.Is<Cart>(c => c.UserId == command.UserId),
            command.RequestedBy,
