...
public class CreateSaleHandlerTests
{
    ...
    public CreateSaleHandlerTests()
    {
        ...
        _handler = new CreateSaleHandler(_cartRepository, _productRepository, _userRepository, _saleRepository, _mapper, _eventNotifier);
    }
    ...
    public async Task Handle_InvalidProductIdInCommand_ThrowsResourceNotFoundException()
    {
        ...
        // Given
        var command = ApplicationSaleTestData.GetValidCreateSaleCommand();
    ...
    [Fact(DisplayName = "Given valid command creates a new Sale")]
    public async Task Handle_ValidRequest_CreatesASale()
    {
        ApplicationSaleResult? SaleResult = null;
        // Given
        var cart = ApplicationCartsTestData.GetValidCart();
        ...
        _userRepository.GetByIdAsync(command.CustomerId, true, Arg.Any<Guid>(), Arg.Any<UserRole>(), Arg.Any<CancellationToken>()).Returns(Task.FromResult(customer));
        _cartRepository.GetByIdAsync(command.CartId, true, Arg.Any<Guid>(), Arg.Any<UserRole>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<Cart>(cart));
        ...
        await _saleRepository.Received(1).AddAsync(Arg.Is<Sale>(c => c.Customer!.Id == command.CustomerId), Arg.Any<CancellationToken>());
        await _eventNotifier.Received(1).NotifyAsync(Arg.Any<SaleCreatedEvent>());
    }
}